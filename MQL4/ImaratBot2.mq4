...
#property description "Imarat Bot 2 - broker-aware XAUUSD EMA and RSI Expert Advisor"

input string InpSymbol="";
input ENUM_TIMEFRAMES InpSignalTimeframe=PERIOD_M15;
input int InpMagicNumber=26090602;
...
string PanelName(const string suffix) { return("IMARAT2_"+IntegerToString(InpMagicNumber)+"_"+suffix); }
int CurrentDayKey()
  {
   datetime now=TimeCurrent();
   return(TimeYear(now)*10000+TimeMonth(now)*100+TimeDay(now));
  }
string DayEquityKey() { return("IMARAT2_DAY_EQUITY_"+IntegerToString(InpMagicNumber)+"_"+IntegerToString(AccountNumber())); }
...
double SpreadPoints(const double spread) { return(spread/PointSize()); }
...
bool IsTradingHour()
  {
   ...
   if(InpTradeStartHour<=InpTradeEndHour) return(hour>=InpTradeStartHour && hour<=InpTradeEndHour);
...
double CalculateLots(const double stopDistance)
  {
   ...
   if(tickSize<=0.0 || tickValue<=0.0 || stopDistance<=0.0 || step<=0.0)
     {
      Print("Imarat Bot 2: invalid broker contract specification for ",g_symbol);
      return(0.0);
     }
   ...
   if(lots<minimum)
     {
      Print("Imarat Bot 2: risk amount is below the broker minimum volume for ",g_symbol);
      return(0.0);
     }
...
int SignalDirection()
  {
   ...
   double fast=iMA(g_symbol,InpSignalTimeframe,InpFastEMA,0,MODE_EMA,PRICE_CLOSE,1);
   double slow=iMA(g_symbol,InpSignalTimeframe,InpSlowEMA,0,MODE_EMA,PRICE_CLOSE,1);
   double rsi=iRSI(g_symbol,InpSignalTimeframe,InpRSIPeriod,PRICE_CLOSE,1);
   if(fast>slow && rsi>=InpRSIBuyMinimum) return(1);
   if(fast<slow && rsi<=InpRSISellMaximum) return(-1);
   return(0);
  }
bool CanOpen(const double spread)
  {
   return(IsTradingHour() && !DailyLossLimitReached() && !HasOurPosition() &&
          SpreadPoints(spread)<=InpMaxSpreadPoints && IsTradeAllowed());
  }
void OpenTrade(const int direction)
  {
   ...
   int ticket=OrderSend(g_symbol,type,lots,price,InpSlippagePoints,0,0,"Imarat Bot 2",InpMagicNumber,0,arrow);
   if(ticket<0) { Print("Imarat Bot 2 OrderSend failed for ",g_symbol,": ",GetLastError()); return; }
   if(!OrderSelect(ticket,SELECT_BY_TICKET)) { Print("Imarat Bot 2 cannot select ticket ",ticket,": ",GetLastError()); return; }
   ...
   if(!OrderModify(ticket,OrderOpenPrice(),NormalizePrice(sl),NormalizePrice(tp),0,arrow))
      Print("Imarat Bot 2 protection update failed for ticket ",ticket,": ",GetLastError());
  }
      ...
      if(OrderType()==OP_BUY && bid-OrderOpenPrice()>=TrailStartPrice())
        {
         ...
         if((OrderStopLoss()==0.0 || newSL>OrderStopLoss()) &&
            ...
            Print("Imarat Bot 2 buy trailing-stop update failed: ",GetLastError());
        }
      if(OrderType()==OP_SELL && OrderOpenPrice()-ask>=TrailStartPrice())
        {
         ...
         if((OrderStopLoss()==0.0 || newSL<OrderStopLoss()) &&
            ...
            Print("Imarat Bot 2 sell trailing-stop update failed: ",GetLastError());
        }
     }
  }
...
void UpdatePanel()
  {
   ...
   SetPanelLine("title",0," IMARAT BOT 2 | XAUUSD ",clrWhite);
   ...
   SetPanelLine("spread",3,StringFormat("ASK - BID  %.0f / %d POINTS",SpreadPoints(spread),InpMaxSpreadPoints),spreadAllowed ? clrLimeGreen : clrTomato);
...
int OnInit()
  {
   ...
   if(!SymbolSelect(g_symbol,true)) { Print("Imarat Bot 2 cannot select symbol ",g_symbol); return(INIT_FAILED); }
