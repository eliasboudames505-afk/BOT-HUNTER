...
string PanelName(const string suffix) { return("BAEA5_"+IntegerToString((int)InpMagicNumber)+"_"+suffix); }
int CurrentDayKey()
  {
   MqlDateTime now;
...
string DayEquityKey() { return("BAEA5_DAY_EQUITY_"+IntegerToString((int)InpMagicNumber)+"_"+IntegerToString((int)AccountInfoInteger(ACCOUNT_LOGIN))); }
...
void OpenTrade(const int direction)
  {
   ...
   bool sent=(direction>0 ? g_trade.Buy(lots,g_symbol,price,NormalizePrice(sl),NormalizePrice(tp),"BrokerAware") :
                              g_trade.Sell(lots,g_symbol,price,NormalizePrice(sl),NormalizePrice(tp),"BrokerAware"));
